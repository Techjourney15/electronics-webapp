...
def personalized_homepage(request):
    ...
    weighted_ids = {}  # product_id (string) -> weight
...
def seller_storefront(request, seller_id):
    ...
    return Response({
        'seller': {
            ...
            'seller_name': seller.user.first_name or seller.user.username,
...
def unclaimed_products(request):
    ...
    Q(seller__isnull=True) | Q(seller__is_dummy=True)
)
...
def add_to_cart(request):
    ...
    if created:
        if product.stock_quantity < quantity:
            return Response(
                {
                    'error': f'Only {product.stock_quantity} '
                              f'unit(s) available'
                },
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    for order in orders:
        ...
        for i in order.items.all():
            item_data = {
                ...
                'quantity': i.quantity,
                'image': i.product.image.url if i.product and i.product.image else None,
                'seller_name': i.seller_business_snapshot or None,
            }
...
def checkout(request):
    ...
    # Check ALL stock before creating the order
    for item in items:
    ...
    total = sum(
        item.product.price_npr * item.quantity
        for item in items
    )
    ...
    for item in items:
        ...
        # Seller information at the moment of purchase
        seller_name = ""
        seller_business = ""
        seller_email = ""
        seller_contact = ""
        ...
        OrderItem.objects.create(
            ...
            price_at_purchase=product.price_npr,
            quantity=item.quantity,

            seller_name_snapshot=seller_name,
            seller_business_snapshot=seller_business,
            seller_email_snapshot=seller_email,
            seller_contact_snapshot=seller_contact,
        )
...
class BuyNowCheckoutView(APIView):
    """
    Creates a pending order from a single product.

    Stock is only decreased after successful Khalti payment
    in verify_payment().
    """
    ...
    def post(self, request):
        ...
        # Check current stock, but DO NOT decrease it here.
        if product.stock_quantity < quantity:
            return Response(
                {
                    'error': f'Not enough stock for '
        ...
        # Default values when product has no seller
        seller_name = ""
        seller_business = ""
        seller_email = ""
        seller_contact = ""
        ...
        with transaction.atomic():
            ...
            order = Order.objects.create(
                user=request.user,
                total_amount=product.price_npr * quantity,
                status='pending',
            )
            ...
            OrderItem.objects.create(
                ...
                # Save seller information at purchase time
                seller_name_snapshot=seller_name,
                seller_business_snapshot=seller_business,
                seller_email_snapshot=seller_email,
                seller_contact_snapshot=seller_contact,
...
def all_orders(request):
    ...
    orders = Order.objects.all().order_by('-created_at').prefetch_related('items').select_related('user')
            ...
            'items': [
                {
                    'product_name': i.product_name_snapshot,
                    'quantity': i.quantity,
                    'seller_business': i.seller_business_snapshot or None,
                }
                for i in order.items.all()
            ],
        })
